...
def roa_entries(obj):
    ...
    for origin in obj.get("origin"):
        if not re.match(r"AS[0-9]+$", origin): continue
        autnum = origin[2:]
...
if __name__ == "__main__":
    ...
    argparser = argparse.ArgumentParser(description="Generate ROA table from route objects")
    argparser.add_argument("--version", help="IP version", type=int, default=6)
    argparser.add_argument("-4", help="IP version 4", dest="version", action="store_const", const=4)
    argparser.add_argument("-6", help="IP version 6", dest="version", action="store_const", const=6)
    ...
    argparser.add_argument("--flush", "-F", help="Flush table before adding entries", action="store_true", default=False)
    argparser.add_argument("--weak-maxlen", "-M", help="Do not enforce strict maximum lengths", action="store_true", default=False)
